...
with 2022-2025 projections and has no barangay data.
...
Metro Manila files). An early matcher downloaded two province zips by mistake;
they were deleted 2026-08-24 to keep them out of `**/*.shp` globs. Extracted
shapefiles live under `data/raw/noah/extracted/`.
...
hospital / 741 clinic). The hospital count feels thin for 13.5M people, so OSM
coverage is partial; the methodology labels these "OSM-mapped facilities" and
lists under-coverage as a limitation. Spillover beyond NCR is expected and gets
...
rows). NCR rows broke three of its conclusions, caught by the join gate
(0/1,712 match on first run):
...
- **The 10 EMBO barangays (336,873 residents) straddle vintages**: the 2020
  ...
  fix they fall out of the join; that's 2.5% of NCR, more than the ±1% gate
  allows. `PCODE_FIXES` in `paths.py` remaps census→boundary code per barangay
...
100 := 100∪25∪5). This enforces the physical definition that a rarer flood
covers at least what a more frequent one does. The 5-yr layer is untouched; the
25-yr layer is unchanged by construction (already nested); only the 100-yr
layer grows. Documented as a methodology note in the app.
